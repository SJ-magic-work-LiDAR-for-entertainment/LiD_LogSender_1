...
void FilesInDir::OpenFiles()
{
	...
	********************/
	...
	for(int i = 0; i < (int)frame_id_.size(); i++)	{ frame_id_[i] = 0; }
...
, num_points_in_this_frame_(0)
{
	...
	********************/
	...
	pos_thinned_.resize(max_points_in_1_frame);
...
void ofApp::setup(){
	/********************
	********************/
	ofSetWindowTitle("LiD:LogSender");
	...
	********************/
	...
	Group_Limit.setup("Limit");
		Group_Limit.add(gui_b_limit_by_ranging_.setup("gui_b_limit_by_ranging_", true));
		...
		Group_Limit.add(gui_send_per_.setup("send_per_", 2.0, 1.0f, 4.0f));
	...
	Group_SoundSync.setup("SoundSync");
#if defined DEBUG_AS_REALTIME
		Group_SoundSync.add(gui_b_send_udp_sound_sync_point_size.setup("send_sound_sync_point_size", false));
#else
		Group_SoundSync.add(gui_b_send_udp_sound_sync_point_size.setup("send_sound_sync_point_size", true));
#endif
	gui_.add(&Group_SoundSync);
}
...
void ofApp::SetupUdp(){
	...
	********************/
...
#if !defined DEBUG_AS_REALTIME
		settings.sendTo("127.0.0.1", 12347); // 通常
#else
		settings.sendTo("127.0.0.1", 12345); // debug as realtime
#endif
	...
	
	{
		...
		udp_send_soundsync_point_size_.Setup(settings);
	}
}
...
void ofApp::update(){
	...
	if(state_ == State::kRun_){
		...
		UpdatePosition();
		PrepAndSendUdp();
		PrepAndSendUdp_SoundSyncPointSize();
		CalElapsedTime();
	}
...
void ofApp::PrepAndSendUdp(){
	...
	for(int ofs = 0, id = 0; ofs < num_points_to_send_; ofs += kSendAtOnce_, id++){
...
#if !defined DEBUG_AS_REALTIME
			snprintf(buf, std::size(buf), "%d,%d,%d,%d,%d,", 1/* grop_id */, num_points_to_send_, num_packets_, id, ofs); // 通常
#else
		...
		for(int i = 0; i < count; i++){
			...
			if(gui_b_limit_by_ranging_)	snprintf(buf, std::size(buf), "%.2f,%.2f,%.2f", positions_[i + ofs].x, positions_[i + ofs].y, positions_[i + ofs].z );
			else						snprintf(buf, std::size(buf), "%.2f,%.2f,%.2f", pos_thinned_[i + ofs].x, pos_thinned_[i + ofs].y, pos_thinned_[i + ofs].z );
...
/******************************
******************************/
void ofApp::PrepAndSendUdp_SoundSyncPointSize(){
	/********************
	********************/
	if(!gui_b_send_udp_sound_sync_point_size) return;
	
	/********************
	********************/
	const int BUF_SIZE = 100;
	
	string message = "/RemoteControl_SoundSyncPointSize,";
	
	char buf[BUF_SIZE];
	snprintf(buf, std::size(buf), "%.3f, %.3f", sound_sync_point_size[dir_id_].h_, sound_sync_point_size[dir_id_].l_);
	
	message += buf;
	
	udp_send_soundsync_point_size_.Send(message.c_str(), message.length());
}
